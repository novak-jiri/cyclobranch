...
void cSequenceDatabaseProxyModel::initialize(QComboBox* rowsfilteroperator, QComboBox* rowsfiltercombobox1, QComboBox* rowsfiltercomparatorcombobox1, QLineEdit* rowsfilterline1, QComboBox* rowsfiltercombobox2, QComboBox* rowsfiltercomparatorcombobox2, QLineEdit* rowsfilterline2) {
...
bool cSequenceDatabaseProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const {
	...
	if (!filterline1->text().isEmpty()) {
		if (col1 == 1) {
			QString qstr = getStringFromPeptideType((ePeptideType)sourceModel()->data(sourceModel()->index(sourceRow, col1, sourceParent)).toInt()).c_str();
	...
	if (!filterline2->text().isEmpty()) {
		if (col2 == 1) {
			QString qstr = getStringFromPeptideType((ePeptideType)sourceModel()->data(sourceModel()->index(sourceRow, col2, sourceParent)).toInt()).c_str();
